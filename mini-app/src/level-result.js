...
export function renderLevelResult(level, result, handlers = {}) {
  ...
  // Также не показываем «ОШИБКА» при идеальном прохождении (3★): результат-целиком празднует,
  // а гневный красный блок ниже сбивает с толку (игрок видит «Идеально!» и тут же «Ошибка»).
  if (action && action !== verdict && stars < 3) {
